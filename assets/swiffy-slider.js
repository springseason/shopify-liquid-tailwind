const swiffyslider = (function () {
  return {
    ...
    initSlider(sliderElement) {
        ...
        .forEach((navElement) =>
          navElement.addEventListener(
            ...
            () =>
              this.slide(
                sliderElement,
                navElement.classList.contains('slider-nav-next'),
              ),
            { passive: true },
          ),
        );
      sliderElement
        .querySelectorAll('.slider-indicators')
      ...
      if (sliderElement.classList.contains('slider-adaptive-color')) {
        const container = sliderElement.querySelector('.slider-container');
        const firstFontColor =
          container.children[0]?.getAttribute('data-fontcolor');
        if (firstFontColor) {
    ...
    handleIndicators(sliderElement) {
      ...
      sliderElement
        .querySelectorAll('.slider-indicators')
        .forEach((scrollIndicatorContainers) => {
          ...
          if (sliderElement.classList.contains('slider-adaptive-color')) {
            const activeSlide = container.children[activeIndicator];
            const fontColor = activeSlide?.getAttribute('data-fontcolor');
            if (fontColor) {
              sliderElement.style.setProperty(
                '--swiffy-slider-nav-light',
                fontColor,
